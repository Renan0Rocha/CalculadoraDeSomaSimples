...
import "./estilo.css";
import { aplicarMascaraCelular, aplicarMascaraCEP, aplicarMascaraCPF, aplicarMascaraTelefone } from "./mascaras";
import { validarCPF, validarFormulario, verificarIdade } from "./validacoes";
...
const Formulario = () => {
  ...
  const handleChange = (e) => {
    ...
    if (name === "cpf") {
      newValue = aplicarMascaraCPF(value);
      if (newValue.length === 14) {
        setErrors((prevErrors) => ({ ...prevErrors, cpf: validarCPF(newValue.replace(/\D/g, "")) ? "" : "CPF inválido" }));
      } else {
        setErrors((prevErrors) => ({ ...prevErrors, cpf: "" }));
      }
    }
    if (name === "telefoneFixo") newValue = aplicarMascaraTelefone(value);
    if (name === "celular") newValue = aplicarMascaraCelular(value);
    if (name === "cep") newValue = aplicarMascaraCEP(value);

    setFormData((prevFormData) => {
      const updatedFormData = { ...prevFormData, [name]: newValue };
      const validationErrors = validarFormulario(updatedFormData, menorDeIdade);
      setErrors((prevErrors) => ({ ...prevErrors, [name]: validationErrors[name] || "" }));
      return updatedFormData;
    });

    if (name === "dataNascimento") {
      const idade = verificarIdade(value);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const validationErrors = validarFormulario(formData, menorDeIdade);
